...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($data)) {
    ...
    if (!empty($sdt) && !empty($ngaysinh) && !empty($hoten) && !empty($trieuchung) && !empty($ngaykham) && !empty($giokham)) {
        ...
        if ($check_result && $check_result->num_rows > 0) {
            ...
            $sql = "INSERT INTO admin_khachhang (hoten, ngaysinh, sdt, trieuchung,ngaykham,giokham, status, note, ketqua, nguon, user_tuvan, created_at) 
                VALUES ('$hoten', '$ngaysinh', '$sdt', '$trieuchung','$ngaykham_formatted','$giokham', 0, '', 0, '$nguon', 0, '$created_at')";
